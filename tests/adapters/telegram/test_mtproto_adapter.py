...
@pytest.mark.asyncio
async def test_realtime_processing_is_restart_safe_and_insert_only_for_duplicates(
    ...
    summary_two = await service.process_source(source_two, channel_id=777000)
    assert source_two.after_position_calls[0] is not None
    assert source_two.after_position_calls[0].last_message_id == 101
    assert summary_two.inserted == 1
    assert summary_two.duplicates == 1
    assert state_repository.apply_calls == 4


@pytest.mark.asyncio
async def test_mtproto_adapter_reconnects_with_backoff(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    adapter = ReadOnlyTelegramMtprotoAdapter(client)
...
@pytest.mark.asyncio
async def test_logging_is_structured_and_redacted(
    migrated_session_factory: sessionmaker[Session],
) -> None:
    stream = StringIO()
    ...
    assert "telegram_catchup_start" in logs
    assert "raw_message_ingested" in logs
    assert "message_parsed" in logs
    assert "state_machine_applied" in logs
    assert "outbox_commands_created" in logs
